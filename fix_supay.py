"""
修复 exchange_scraper.py 中的 scrape_supay 函数
"""
import re

# 新的 scrape_supay 函数代码
new_supay_code = '''    def scrape_supay(self):
...
# 找到并替换 scrape_supay 函数
# 匹配从 "def scrape_supay" 到下一个 "def " 开始的位置
pattern = r'    def scrape_supay\(self\):.*?(?=    def scrape_moneychase)'
match = re.search(pattern, content, re.DOTALL)

if match:
    print("Found scrape_supay function, replacing...")
    new_content = content[:match.start()] + new_supay_code + '\n' + content[match.end():]
    
    with open('exchange_scraper.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print("Successfully replaced scrape_supay function!")
else:
    print("Could not find scrape_supay function to replace")
